...
for exc in exch:
    ...
    if intraday == 1:
        ...
        hour_of_day, avg_returns_hour, low_returns_hour, upper_returns_hour = dis.cyclical_average(
            time_list_hours_clean, returns_hours_clean, frequency="h")
        hour_of_day, avg_volumes_hour, low_volumes_hour, upper_volumes_hour = dis.cyclical_average(
            time_list_hours_clean, log_volumes_hours_clean, frequency="h")
        hour_of_day, avg_spread_hour, low_spread_hour, upper_spread_hour = dis.cyclical_average(time_list_hours_clean,
                                                                                                spread_hours_clean,
                                                                                                frequency="h")
        hour_of_day, avg_rvol_hour, low_rvol_hour, upper_rvol_hour = dis.cyclical_average(time_list_hours_clean,
                                                                                          rvol_hours_clean,
                                                                                          frequency="h")

        hour_of_day, avg_illiq_hour, low_illiq_hour, upper_illiq_hour = dis.cyclical_average(time_list_hours_clean,
                                                                                             illiq_hours_clean,
                                                                                             frequency="h")

        hour_of_day, avg_log_illiq_hour, low_log_illiq_hour, upper_log_illiq_hour = dis.cyclical_average(illiq_hours_time,
                                                                                                         log_illiq_hours_clean,
                                                                                                         frequency="h")

        plot.intraday(avg_returns_hour, low_returns_hour, upper_returns_hour, title="Return" + exc_name, perc=1,
                      ndigits=2, yzero=1)
        ...
        plot.intraday(avg_rvol_hour, low_rvol_hour, upper_rvol_hour, title="Log_RVol" + exc_name, perc=1, logy=1,
                      ndigits=0)
        plot.intraday(avg_log_illiq_hour, low_log_illiq_hour, upper_log_illiq_hour, title="Log_ILLIQ" + exc_name, perc=0,
                      ndigits=3, logy=0)  # Skulle helst brukt vanlig illiq med log-skala i stedet
        plot.intraday(avg_illiq_hour, low_illiq_hour, upper_illiq_hour, title="ILLIQ" + exc_name,
                      perc=1, ndigits=2)
    ...
    if intraweek == 1:
        ...
        plot.intraweek(avg_illiq_day_clean, low_illiq_day_clean, upper_illiq_day_clean,
                       title="Log_ILLIQ" + exc_name, perc=1,
                       weekends=1, logy=1, ndigits=3)
        plot.intraweek(avg_log_volume_day, low_log_volume_day, upper_log_volume_day, title="Log_Volume" + exc_name,
                       perc=0,
                       weekends=1)  # Hva faen gjør vi med y-aksen på denne?
        plot.intraweek(avg_spread_day_clean, low_spread_day_clean, upper_spread_day_clean,
                       title="Spread_clean" + exc_name,
                       perc=1, weekends=1)
