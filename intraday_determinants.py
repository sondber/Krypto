...
for exc in exchanges:
    ...
    if spread_determinants == 1:
        ...
        # 6 lage benchmark
        Y, X_benchmark, hours_to_remove = rs.benchmark_hourly(Y, time_listH, hours_in_period=hours_in_period, prints=benchmark_prints)
        ...
        for i in range(np.size(coeff_matrix, 0)):
            for j in range(np.size(coeff_matrix, 1)):
                print("{0:5.2f}".format(coeff_matrix[i,j]), end="  ")
            print()
    ...
    if illiq_determinants == 1:
        ...
        print()
        Y = illiqH
        n_cols = 11  # 11 for BAS og for ILLIQ
        ...
        first_col_entries = ['$illiq_{t-1}$', '$illiq_{t-24}$','$illiq^{D}$', '$|r_t[$', '$|r_{t-1}|$', '$v_{t}$',
                             '$v_{t-1}$', '$gv_{t}$', '$gv_{t-1}$', '$rv_{t}$', '$rv_{t-1}$', '\\textit{\\# Obs.}', '$R^2$', '\\textit{AIC}']
